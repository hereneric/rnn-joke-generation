...
def pad_sequences(data, max_length):
    ...
    Args:
        data: is a list of (sentence, labels) tuples. @sentence is a list
            ...
            @n_features features. For example, "Chris/PER
    ...
    for sentence, labels in data:
        ...
        if length >= max_length:
            ...
            n_paddings = max_length - length
            ret.append((sentence + [zero_vector] * n_paddings,
                labels + [zero_label] * n_paddings,
                [True] * length + [False] * n_paddings))
        ### END YOUR CODE ###
    return ret
...
class RNNModel(NERModel):
    ...
    def add_loss_op(self, preds):
        ...
        loss = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(mask_preds, mask_labels))
        ### END YOUR CODE
        return loss
